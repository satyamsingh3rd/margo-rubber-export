 ...
 *
 * Only `CompoundGuide` genuinely needs the client, for its tab selector.
 * The rest are exported from the same file so the page has one import for
 * this comp's vocabulary, and they render identically on the server.
 */
  ...
  return (
    <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {items.map((a) => (
        ...
        >
          {/* Placeholder frame until the four production photographs land.
              Sized to the comp's 4:3 so swapping an image in does not move
              anything below it. */}
          <div className="bg-surface-4 relative grid aspect-[4/3] place-items-center">
            <Icon
